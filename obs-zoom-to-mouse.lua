...
local translations = {
    en = {
        description = "Zoom the selected display-capture source to focus on the mouse",
        language = "Language",
        zoom_source = "Zoom Source",
        refresh_sources = "Refresh zoom sources",
        refresh_sources_desc = "Click to re-populate Zoom Sources dropdown with available sources",
        zoom_factor = "Zoom Factor",
        zoom_speed = "Zoom Speed",
        auto_follow = "Auto follow mouse",
        auto_follow_desc = "When enabled mouse tracking will auto-start when zoomed in without waiting for tracking toggle hotkey",
        follow_outside = "Follow outside bounds",
        follow_outside_desc = "When enabled the mouse will be tracked even when the cursor is outside the bounds of the zoom source",
        follow_speed = "Follow Speed",
        follow_border = "Follow Border",
        lock_sensitivity = "Lock Sensitivity",
        auto_lock = "Auto Lock on reverse direction",
        auto_lock_desc = "When enabled moving the mouse to edge of the zoom source will begin tracking,\nbut moving back towards the center will stop tracking similar to panning the camera in a RTS game",
        allow_all = "Allow any zoom source",
        allow_all_desc = "Enable to allow selecting any source as the Zoom Source\nYou MUST set manual source position for non-display capture sources",
        manual_position = "Set manual source position",
        manual_position_desc = "When enabled the specified size/position settings will be used for the zoom source instead of the auto-calculated ones",
        scale_x_desc = "Usually 1 - unless you are using a scaled source",
        scale_y_desc = "Usually 1 - unless you are using a scaled source",
        monitor_width_desc = "X resolution of your monitor",
        monitor_height_desc = "Y resolution of your monitor",
        remote_listener = "Enable remote mouse listener",
        remote_listener_desc = "When enabled a UDP socket server will listen for mouse position messages from a remote client",
        port_desc = "You must restart the server after changing the port (Uncheck then re-check 'Enable remote mouse listener')",
        poll_desc = "You must restart the server after changing the poll delay (Uncheck then re-check 'Enable remote mouse listener')",
        more_info = "More Info",
        more_info_desc = "Click to show help information (via the script log)",
        debug_logging = "Enable debug logging",
        debug_logging_desc = "When enabled the script will output diagnostics messages to the script log (useful for debugging/github issues)",
        none = "<None>"
    },
    tr = {
        description = "Seçili ekran yakalama kaynağını mouse'a odaklamak için yakınlaştır",
        language = "Dil",
        zoom_source = "Yakınlaştırma Kaynağı",
        refresh_sources = "Kaynakları yenile",
        refresh_sources_desc = "Yakınlaştırma Kaynakları listesini yenilemek için tıklayın",
        zoom_factor = "Yakınlaştırma Oranı",
        zoom_speed = "Yakınlaştırma Hızı",
        auto_follow = "Otomatik mouse takibi",
        auto_follow_desc = "Etkinleştirildiğinde, yakınlaştırıldığında takip kısayol tuşunu beklemeden mouse takibi otomatik başlar",
        follow_outside = "Sınır dışında takip et",
        follow_outside_desc = "Etkinleştirildiğinde, imleç yakınlaştırma kaynağının sınırları dışında olsa bile takip edilir",
        follow_speed = "Takip Hızı",
        follow_border = "Takip Sınırı",
        lock_sensitivity = "Kilit Hassasiyeti",
        auto_lock = "Ters yönde otomatik kilitle",
        auto_lock_desc = "Etkinleştirildiğinde, mouse'u yakınlaştırma kaynağının kenarına taşımak takibi başlatır,\nancak merkeze geri dönmek takibi durdurur (RTS oyunlarındaki kamera kaydırma gibi)",
        allow_all = "Tüm kaynaklara izin ver",
        allow_all_desc = "Herhangi bir kaynağı Yakınlaştırma Kaynağı olarak seçmeye izin verir\nEkran yakalama olmayan kaynaklar için manuel kaynak konumu AYARLANMALIDIR",
        manual_position = "Manuel kaynak konumu ayarla",
        manual_position_desc = "Etkinleştirildiğinde, otomatik hesaplanan değerler yerine belirtilen boyut/konum ayarları kullanılır",
        scale_x_desc = "Genellikle 1 - ölçeklenmiş kaynak kullanmıyorsanız",
        scale_y_desc = "Genellikle 1 - ölçeklenmiş kaynak kullanmıyorsanız",
        monitor_width_desc = "Monitörünüzün X çözünürlüğü",
        monitor_height_desc = "Monitörünüzün Y çözünürlüğü",
        remote_listener = "Uzak mouse dinleyiciyi etkinleştir",
        remote_listener_desc = "Etkinleştirildiğinde, uzak istemciden mouse konum mesajlarını dinleyen bir UDP soket sunucusu başlatılır",
        port_desc = "Port değiştirdikten sonra sunucuyu yeniden başlatmalısınız ('Uzak mouse dinleyiciyi etkinleştir' seçeneğini kapatıp açın)",
        poll_desc = "Yoklama gecikmesini değiştirdikten sonra sunucuyu yeniden başlatmalısınız ('Uzak mouse dinleyiciyi etkinleştir' seçeneğini kapatıp açın)",
        more_info = "Daha Fazla Bilgi",
        more_info_desc = "Yardım bilgilerini göstermek için tıklayın (script günlüğü üzerinden)",
        debug_logging = "Hata ayıklama günlüğünü etkinleştir",
        debug_logging_desc = "Etkinleştirildiğinde, script tanılama mesajlarını script günlüğüne yazar (hata ayıklama/github sorunları için kullanışlı)",
        none = "<Yok>"
    }
}
...
function T(key)
    local lang = translations[current_language] or translations["en"]
    return lang[key] or translations["en"][key] or key
end
...
-- Helper functions for OBS 30+ compatibility
-- OBS 30 removed obs_sceneitem_get_info/set_info and replaced with info2 variants
-- but Lua bindings don't expose obs_transform_info2() constructor
local use_info2_api = (major >= 30)

-- Wrapper functions to handle API differences
function get_sceneitem_info(item)
    if use_info2_api then
        local info = obs.obs_transform_info()
        -- Try the new API first, fall back to creating a compatible table
        local success, err = pcall(function()
            obs.obs_sceneitem_get_info2(item, info)
        end)
        if not success then
            -- If info2 fails, the struct might be incompatible
            -- Create a manual info table from individual properties
            info = obs.obs_transform_info()
            pcall(function() obs.obs_sceneitem_get_info(item, info) end)
        end
        return info
    else
        local info = obs.obs_transform_info()
...
function set_sceneitem_info(item, info)
    if use_info2_api then
        local success = pcall(function()
            obs.obs_sceneitem_set_info2(item, info)
        end)
        if not success then
            pcall(function() obs.obs_sceneitem_set_info(item, info) end)
        end
    else
...
function release_sceneitem()
    ...
    if sceneitem ~= nil then
        ...
        if sceneitem_info_orig ~= nil then
            ...
            set_sceneitem_info(sceneitem, sceneitem_info_orig)
...
function refresh_sceneitem(find_newest)
    ...
    if sceneitem ~= nil then
        ...
        -- Use helper functions for OBS version compatibility
        sceneitem_info_orig = get_sceneitem_info(sceneitem)
        sceneitem_info = get_sceneitem_info(sceneitem)
...
function script_description()
    return "Zoom the selected display-capture source to focus on the mouse\n" ..
           "Seçili ekran yakalama kaynağını mouse'a odaklamak için yakınlaştırır\n\n" ..
           "⚠️ After changing language, please restart OBS to apply.\n" ..
           "⚠️ Dil değiştirdikten sonra, uygulamak için lütfen OBS'yi yeniden başlatın."
end
...
function script_properties()
    ...
    -- Language selection
    local lang_list = obs.obs_properties_add_list(props, "language", T("language"), obs.OBS_COMBO_TYPE_LIST,
        obs.OBS_COMBO_FORMAT_STRING)
    obs.obs_property_list_add_string(lang_list, "English", "en")
    obs.obs_property_list_add_string(lang_list, "Türkçe", "tr")
    obs.obs_property_set_modified_callback(lang_list, on_language_changed)
    ...
    local sources_list = obs.obs_properties_add_list(props, "source", T("zoom_source"), obs.OBS_COMBO_TYPE_LIST,
        obs.OBS_COMBO_FORMAT_STRING)

    populate_zoom_sources(sources_list)

    local refresh_sources = obs.obs_properties_add_button(props, "refresh", T("refresh_sources"),
        function()
            populate_zoom_sources(sources_list)
            monitor_info = get_monitor_info(source)
            return true
        end)
    obs.obs_property_set_long_description(refresh_sources, T("refresh_sources_desc"))

    -- Add the rest of the settings UI
    local zoom = obs.obs_properties_add_float(props, "zoom_value", T("zoom_factor"), 1, 5, 0.5)
    local zoom_speed = obs.obs_properties_add_float_slider(props, "zoom_speed", T("zoom_speed"), 0.01, 1, 0.01)
    local follow = obs.obs_properties_add_bool(props, "follow", T("auto_follow") .. " ")
    obs.obs_property_set_long_description(follow, T("auto_follow_desc"))

    local follow_outside_bounds = obs.obs_properties_add_bool(props, "follow_outside_bounds", T("follow_outside") .. " ")
    obs.obs_property_set_long_description(follow_outside_bounds, T("follow_outside_desc"))

    local follow_speed = obs.obs_properties_add_float_slider(props, "follow_speed", T("follow_speed"), 0.01, 1, 0.01)
    local follow_border = obs.obs_properties_add_int_slider(props, "follow_border", T("follow_border"), 0, 50, 1)
    local safezone_sense = obs.obs_properties_add_int_slider(props,
        "follow_safezone_sensitivity", T("lock_sensitivity"), 1, 20, 1)
    local follow_auto_lock = obs.obs_properties_add_bool(props, "follow_auto_lock", T("auto_lock") .. " ")
    obs.obs_property_set_long_description(follow_auto_lock, T("auto_lock_desc"))

    local allow_all = obs.obs_properties_add_bool(props, "allow_all_sources", T("allow_all") .. " ")
    obs.obs_property_set_long_description(allow_all, T("allow_all_desc"))
    ...
    local override = obs.obs_properties_add_group(props, "use_monitor_override", T("manual_position") .. " ",
        obs.OBS_GROUP_CHECKABLE, override_props)

    obs.obs_property_set_long_description(override_label, T("manual_position_desc"))
    obs.obs_property_set_long_description(override_sx, T("scale_x_desc"))
    obs.obs_property_set_long_description(override_sy, T("scale_y_desc"))
    obs.obs_property_set_long_description(override_dw, T("monitor_width_desc"))
    obs.obs_property_set_long_description(override_dh, T("monitor_height_desc"))
    ...
    if socket_available then
        ...
        local socket = obs.obs_properties_add_group(props, "use_socket", T("remote_listener") .. " ",
            obs.OBS_GROUP_CHECKABLE, socket_props)

        obs.obs_property_set_long_description(r_label, T("remote_listener_desc"))
        obs.obs_property_set_long_description(r_port, T("port_desc"))
        obs.obs_property_set_long_description(r_poll, T("poll_desc"))
    ...
    local help = obs.obs_properties_add_button(props, "help_button", T("more_info"), on_print_help)
    obs.obs_property_set_long_description(help, T("more_info_desc"))

    local debug = obs.obs_properties_add_bool(props, "debug_logs", T("debug_logging") .. " ")
    obs.obs_property_set_long_description(debug, T("debug_logging_desc"))
...
function on_language_changed(props, prop, settings)
    current_language = obs.obs_data_get_string(settings, "language")
    return true
end
...
function populate_zoom_sources(list)
    ...
    if sources ~= nil then
        local dc_info = get_dc_info()
        obs.obs_property_list_add_string(list, T("none"), "obs-zoom-to-mouse-none")
